...
public class ProduitDataService {
    ...
    @Transactional
	public void sauvegarder(ProduitData entry, Date date) {
		...
		DbProduitData dbEntry = new DbProduitData(produit, entry.getDesignation(), entry.getTva(), entry.getPrixUnitaireHT(), date);
    ...
    public List<ProduitData> chargerEntreesActuelles() {
		return produitDataRepository.findAllWithoutDateEnd().stream()
				.map(dbEntry -> new ProduitData(dbEntry.getProduit().getProduitId(), dbEntry.getDesignation(),
